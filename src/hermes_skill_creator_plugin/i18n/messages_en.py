...
ADVISORY_CAP_EN = (
    ...
    "Hermes checkout. Run `hermes-skill-creator-patch` to raise it."
)
...
REPORT_NO_PROFILES = (
    "[en] no profiles found / [hu] nem találhatók profilok"
)
...
EN_MESSAGES = MappingProxyType({
    ...
    "profiles_opt_audit": (
        "Run audit only; do not perform any writes (alias for the default)."
    ),
    "profiles_opt_profile": (
        "Restrict the run to a single profile (default: every profile)."
    ),
    "profiles_opt_json": (
        "Write the deterministic JSON report to PATH "
        "(default: ./profile-audit.json)."
    ),
    "profiles_opt_yes": (
        "Suppress the interactive TTY confirmation (CI / non-TTY runs)."
    ),
    "profiles_opt_skip_install": (
        "Audit only; do not call hub_install_or_update."
    ),
    "profiles_opt_frozen_time": (
        "Pin the report's generated_at to a stable ISO 8601 UTC string."
    ),
    ...
    "profiles_msg_audit_default": (
        "Default mode: dry-run (use --apply to perform writes)."
    ),
    "profiles_msg_applying": "Applying per-profile flip...",
    "profiles_msg_profile_audit": (
        "profile={name} current_disabled={disabled} "
    ...
    "profiles_msg_cache_warn": (
        "profile={name} clear_skills_system_prompt_cache raised: "
        "{err} (continuing)"
    ),
    "profiles_msg_hub_error": (
        "profile={name} hub install failed: {err} (continuing)."
    ),
    ...
    "profiles_msg_no_profiles": (
        "No profiles found (default + named). Nothing to audit."
    ),
