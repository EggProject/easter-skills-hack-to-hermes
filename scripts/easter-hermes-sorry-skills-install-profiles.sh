#!/usr/bin/env bash
# scripts/easter-hermes-sorry-skills-install-profiles.sh — transparent wrapper around the .pyz entry point
...
exec "${PYZ_PATH}" -c "import sys; sys.argv[0] = 'easter-hermes-sorry-skills-install-profiles'; from easter_hermes_sorry_skills.cli_profiles import main; main(standalone_mode=True)" "$@"
