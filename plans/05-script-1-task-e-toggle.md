...
| `E6.skill_manage_schema_desc` | `tools/skill_manager_tool.py` | `SKILL_MANAGE_SCHEMA` `description="""...` | "Create, edit, patch, delete, write_file, or remove_file" (the `description=` argument's first line; OPTIONAL site) | APPEND a new sentence to the existing description: "skill-creator, when installed, provides authoring guidance only. Use skill_manage to persist all skill files." Do NOT modify the tool's actions, signature, or any non-description field. | +0 (additive only) |
...
The verbatim sentence Script #1 appends to the `SKILL_MANAGE_SCHEMA` `description=...` argument:
...
- `test_e6_appends_only` — locate `SKILL_MANAGE_SCHEMA` in `tools/skill_manager_tool.py`; assert the original `description="""Create, edit, patch, delete, write_file, or remove_file` text is preserved and the appended sentence is present. Assert no other field of the schema changed (action enum, required fields, etc.).
